...
def main():
    ...
    layout = [
        [psg.Text(size=(70, 1))],
        [psg.Text('Selecione a pasta', size=(30, 1), font='Arial 12', justification='center',
                  text_color='black',key="Text")],
        [psg.FolderBrowse('Clique para selecionar',key="-FOLDER-")],
        [psg.ProgressBar(50, orientation='h',visible=False, size=(30, 10), key='progbar', bar_color=['brown4', 'White'],
                          border_width=1)],
        [psg.Button('Unificar',border_width=2,size=(15,1))]
        ]
    win = psg.Window('Unifica XML', layout, size=(500, 200),element_justification='c')
    value_progbar = 0
    while True:
        ...
        if event is None:
            ...
            for dirpath, dirnames, filenames in os.walk(pasta_raiz):
                # dirpath =  caminho
                win['progbar'].update_bar(value_progbar)
                ...
                win['Text'].update('Arquivo gerado com sucesso')
                sleep(2)
                win.close()
