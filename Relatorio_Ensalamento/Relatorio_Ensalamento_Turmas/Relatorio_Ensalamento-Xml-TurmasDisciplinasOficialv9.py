...
def armazenaDadosAscXml(file_name_asc_xml,file_json_turmasfic,contador, file_json_escolas):
...
def gerarRelatorio(file_name_turma_disciplina,file_name_asc_xml,file_json_sigla,file_name_capacidade,file_json_turmasfic,contador, file_json_escolas, file_json_tipos):
    siglas = dict(file_json_sigla)
    ...
    array_asc_xml = armazenaDadosAscXml(file_name_asc_xml,file_json_turmasfic,contador, file_json_escolas)
            ...
            for x in range(0, quantidade_horarios + 1):
                ...
                for i in range(0, quantidade):
                    ...
                    dict_relatorio_final[key_final] = {
                        'Estabeleciomento': 'PUCPR - CURITIBA',
    ...
    worksheet.add_table(0, 0, row - 1, 24, {'style': 'Table Style Light 1', 'columns': [
        {'header': 'Estabeleciomento'},
        {'header': 'Escola'},
        {'header': 'CR Curso'},
        {'header': 'Curso'},
        {'header': 'Período'},
...
def main(arquivo_carga_horaria,arquivo_capacidade, arquivo_xml):
    ...
    with open('Dicionarios/siglas.json', 'r', encoding="utf-8") as j:
        file_json_sigla = json.loads(j.read())

    with open('Dicionarios/turmasfic.json', 'r', encoding="utf-8") as j:
        file_json_turmasfic = json.loads(j.read())

    with open('Dicionarios/escolas.json', 'r', encoding="utf-8") as j:
        file_json_escolas = json.loads(j.read())

    with open('Dicionarios/tipos.json', 'r', encoding="utf-8") as j:
        file_json_tipos = json.loads(j.read())

    win['progbar'].update_bar(1)
    gerarRelatorio(file_name_turma_disciplina,file_name_asc_xml,file_json_sigla,file_name_capacidade,file_json_turmasfic,contador, file_json_escolas, file_json_tipos)
